      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {Object.entries(primaryMetricsData).map(([key, metric]) => {
            ...
            return (
              <div key={key} className="bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 p-6 hover:shadow-lg transition-shadow">
                ...
                <div className="mb-3">
                  {isEditing && isDataEntryMode ? (
                    <EditableMetricValue 
                      ...
                      onSave={(newValue, notes, timePeriod) => handleMetricEdit(key, newValue, notes, timePeriod)}
                      onCancel={() => setEditingMetric(null)}
                    />
                  ) : (
                    <div 
                      className={`text-3xl font-bold text-slate-900 dark:text-slate-100 mb-2 ${
                        isDataEntryMode ? 'cursor-pointer hover:bg-yellow-100 rounded px-2 py-1' : ''
                      }`}
                      onClick={() => {
                        if (isDataEntryMode) {
                ...
                <div className="space-y-2">
                  ...
                  <div className="text-xs text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-700/50 rounded px-2 py-1">
